/**
 * CSE Pulse — Cloudflare Worker v4.2
 * ─────────────────────────────────────────────────────────────
 * CHANGES in v4.2 (from v4.1):
 *   - Model: claude-sonnet-4-20250514 (was claude-sonnet-4-20250514 with old alias)
 *   - AI_MODEL env override support added
 *   - /api/crisis: double JSON parse-guard + better stale fallback
 *   - Cors header for cse-investing.lk noted in comments
 *   - Health now returns version 4.2.0 + model info
 *
 * Routes:
 *   GET  /api/health       → health check (v4.2)
 *   GET  /api/status       → market open/closed
 *   GET  /api/market       → ASPI + all stocks (90s KV cache)
 *   GET  /api/movers       → gainers / losers / volume
 *   GET  /api/sectors      → sector heatmap
 *   GET  /api/forex        → USD/LKR + major currencies
 *   GET  /api/news         → Sri Lanka news (3 min KV cache)
 *   GET  /api/worldnews    → World news (5 min KV cache)
 *   POST /api/stocksense   → Stock Sense LK AI
 *   POST /api/crisis       → Global Crisis → SL Chain Reaction (6h cache)
 *   GET  /api/cache/clear  → admin cache clear
 *
 * Secrets to set in Cloudflare Dashboard:
 *   ANTHROPIC_API_KEY  → your Anthropic API key
 *   ALPHA_VANTAGE_KEY  → forex fallback
 *   MARKETAUX_KEY      → news API
 *   ADMIN_TOKEN        → cache clear auth
 *   ALLOWED_ORIGIN     → https://cse-investing.lk
...
function corsHeaders(env) {
  return {
    'Access-Control-Allow-Origin':  env.ALLOWED_ORIGIN || 'https://cse-investing.lk',
...
// ═══════════════════════════════════════════════════
//  RSS PARSER — enhanced image extraction
// ═══════════════════════════════════════════════════
  ...
  const results = await Promise.allSettled(
    feeds.map(async feed => {
      const r = await fetch(feed.url, {
        headers: { 'User-Agent': 'Mozilla/5.0 CSEPulse/4.2', 'Accept': 'application/rss+xml, application/xml, text/xml, */*' },
        cf: { cacheTtl: 60 },
      });
...
Sri Lanka context: IMF EFF programme (5 of 6 reviews done, final review due May 2026), ASPI ~12,847, USD/LKR ~329, forex reserves $5.6B, key sectors: Hotels & Leisure (JKH, SPEN, AHUN), Banking (COMB, SAMP, HNB), Plantations (CARS, KLNR), IT/BPO (CALT), Logistics (EXPO).
...
JSON schema (all fields required):
{
  ...
  "meters": {
    "lkrPressure": {"level": "HIGH", "pct": 82},
    "importRisk": {"level": "ELEVATED", "pct": 74},
    "exportRevenue": {"level": "POSITIVE", "pct": 62},
    "sentiment": {"level": "CAUTIOUS", "pct": 42}
  },
  "chainSteps": [
    { "num": 1, "icon": "🌍", "title": "step title", "desc": "1-sentence impact", "impact": "+/-X%", "type": "neg|pos|neu" }
  ],
  "sectors": [
    { "name": "Hotels & Leisure", "icon": "🏨", "impact": -3.5, "type": "neg|pos|neu", "desc": "1-sentence sector impact", "stocks": [
      { "sym": "JKH.N0000", "company": "John Keells", "priceChg": "-2 to -5%", "signal": "buy|hold|sell", "reason": "brief reason" }
    ]}
  ],
  "tariffCascade": {
    "steps": [
      { "num": 1, "label": "Step label", "desc": "What happens", "impact": "+/-X%", "type": "neg|pos|neu" }
    ],
    "affectedStocks": [
      { "sym": "JKH.N0000", "verdict": "BUY|HOLD|SELL", "type": "pos|neg|neu", "reason": "brief" }
    ]
  },
  "riskGauges": [
    { "title": "Currency Risk", "score": 72, "color": "#ff453a", "label": "High" }
  ],
  "narrative": "150-200 word investor strategy. What CSE investors should do. End with: ⚠️ Not financial advice."
}`;
  ...
  try {
    ...
    const result = { ...parsed, updatedAt: Date.now(), generatedBy: d.model };
...
export default {
  async fetch(request, env) {
    ...
    try {
      if (path === '/api/health')                           return okRes({ service: 'CSE Pulse Worker', version: '4.2.0', model: env.AI_MODEL || DEFAULT_MODEL, ts: Date.now() }, env);
      ...
      if (path === '/api/crisis'    && method === 'POST')   return handleCrisis(request, env);
